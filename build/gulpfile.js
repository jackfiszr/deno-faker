...
Object.keys(locales).forEach(function(locale, i) {
  ...
  gulp.task(locale + "Task", function() {
    ...
    return gulp.src("./" + locale + ".js")
      ...
      .pipe(
        rename(
          "../examples/browser/locales/" + locale + "/" + "faker." + locale +
            "min.js"
        )
      );
  });
...
gulp.task("nodeLocalRequires", function(cb) {
  ...
  for (var locale in locales) {
    var localeFile = path.normalize(
      __dirname + "/../locale/" + locale + ".js"
    );
    var localeRequire = "";
    localeRequire += "var Faker = require('../lib');\n";
    localeRequire += "var faker = new Faker({ locale: '" + locale +
      "', localeFallback: 'en' });\n";
    // TODO: better fallback support
    localeRequire += "faker.locales['" + locale +
      "'] = require('../lib/locales/" + locale + "');\n";
    localeRequire += "faker.locales['" + "en" +
      "'] = require('../lib/locales/" + "en" + "');\n";

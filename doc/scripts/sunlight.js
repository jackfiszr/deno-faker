...
(function(window, document, undefined) {
  ...
  isIe = !+"\v1", EOL = isIe ? "\r" : "\n", EMPTY = function() {
    return null;
  }, HIGHLIGHTED_NODE_COUNT = 0, DEFAULT_LANGUAGE = "plaintext",
    DEFAULT_CLASS_PREFIX = "sunlight-", //global sunlight variables
  defaultAnalyzer, getComputedStyle, globalOptions = {
    tabWidth: 4,
    classPrefix: DEFAULT_CLASS_PREFIX,
    showWhitespace: false,
    maxHeight: false
  }, languages = {}, languageDefaults = {}, events = {
    beforeHighlightNode: [],
    beforeHighlight: [],
    beforeTokenize: [],
    afterTokenize: [],
  ...
  function createCodeReader(text) {
    var index = 0, line = 1, column = 1, length, EOF = undefined, currentChar,
      nextReadBeginsLine;
    ...
    return {
      toString: function() {
        return "length: " + length + ", index: " + index + ", line: " + line +
          ", column: " + column + ", current: [" + currentChar + "]";
      },
      ...
      read: function(count) {
        var value = getCharacters(count), newlineCount, lastChar;
  ...
  function contains(arr, value, caseInsensitive) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (
        caseInsensitive && typeof (arr[i]) === "string" &&
        typeof (value) === "string" &&
  ...
  function createProceduralRule(
    ...
    return function(tokens) {
      var tokenIndexStart = startIndex, j, expected, actual;
  ...
  function matchWord(context, wordMap, tokenName, doNotRead) {
    var current = context.reader.current(), i, word, peek,
    ...
    for (i = 0; i < wordMap.length; i++) {
      ...
      if (word === peek || wordMap[i].regex.test(peek)) {
        return context.createToken(
          tokenName,
          context.reader.current() + context.reader[doNotRead
            ? "peek"
            : "read"](word.length - 1),
          line,
          column
  ...
  function defaultNumberParser(context) {
    var current = context.reader.current(), number,
      line = context.reader.getLine(), column = context.reader.getColumn(),
      allowDecimal = true, peek;
    ...
    while ((peek = context.reader.peek()) !== context.reader.EOF) {
      if (!/[A-Za-z0-9]/.test(peek)) {
        if (
          peek === "." && allowDecimal && /\d$/.test(context.reader.peek(2))
        ) {
  ...
  function fireEvent(eventName, highlighter, eventContext) {
    var delegates = events[eventName] || [], i;
  ...
  Highlighter.prototype = (function() {
    var parseNextToken = (function() {
      function isIdentMatch(context) {
        return context.language.identFirstLetter &&
      ...
      function parseIdent(context) {
        var ident, peek, line = context.reader.getLine(),
      ...
      function parseScopes(context) {
        var current = context.reader.current(), tokenName, specificScopes, j,
          opener, line, column, continuation, value;
        ...
        for (tokenName in context.language.scopes) {
          ...
          for (j = 0; j < specificScopes.length; j++) {
            ...
            continuation = getScopeReaderFunction(
              specificScopes[j],
              tokenName
            );
      ...
      function parseCustomRules(context) {
        var customRules = context.language.customParseRules, i, token;
    ...
    function getScopeReaderFunction(scope, tokenName) {
      var escapeSequences = scope[2] || [], closerLength = scope[1].length,
        closer = typeof (scope[1]) === "string"
          ? new RegExp(regexEscape(scope[1]))
          : scope[1].regex, zeroWidth = scope[3] || false;
      ...
      return function(
        ...
        function process(processCurrent) {
          ...
          var peekValue, current = context.reader.current(), i;

          for (i = 0; i < escapeSequences.length; i++) {
            peekValue = (processCurrent ? current : "") +
        ...
        if (!processCurrent || process(true)) {
          while (
            context.reader.peek() !== context.reader.EOF && process(false)
          ) {
          }
        }
    ...
    function switchBackFromEmbeddedLanguageIfNecessary(context) {
      var current = last(context.embeddedLanguageStack), lang;
    ...
    function tokenize(unhighlightedCode, language, partialContext, options) {
      var tokens = [], context, continuation, token;

      fireEvent(
        "beforeTokenize",
        this,
        { code: unhighlightedCode, language: language }
      );
      context = {
      ...
      fireEvent(
        "afterTokenize",
        this,
        { code: unhighlightedCode, parserContext: context }
      );
      return context;
    }

    function createAnalyzerContext(parserContext, partialContext, options) {
      var nodes = [], prepareText = function() {
        var nbsp, tab;
        if (options.showWhitespace) {
          nbsp = String.fromCharCode(0xB7);
          tab = new Array(options.tabWidth).join(String.fromCharCode(0x2014)) +
        ...
        return function(token) {
          var value = token.value.split(" ").join(nbsp), tabIndex,
            lastNewlineColumn, actualColumn, tabLength;
          ...
          while ((tabIndex = value.indexOf("\t")) >= 0) {
            ...
            actualColumn = lastNewlineColumn === -1
              ? tabIndex
    ...
    function highlightText(unhighlightedCode, languageId, partialContext) {
      var language = languages[languageId], analyzerContext;
    ...
    function analyze(analyzerContext, startIndex) {
      ...
      if (analyzerContext.tokens.length > 0) {
        ...
        for (i = startIndex; i < analyzerContext.tokens.length; i++) {
          language = languages[analyzerContext.tokens[i].language] ||
            languages[DEFAULT_LANGUAGE];
          ...
          analyzer = analyzerContext.getAnalyzer.call(analyzerContext) ||
    ...
    return {
      ...
      highlightNode: function highlightRecursive(node) {
        var match, languageId, currentNodeCount, j, nodes, k, partialContext,
  ...
  window.Sunlight.registerLanguage(
    DEFAULT_LANGUAGE,
    { punctuation: /(?!x)x/, numberParser: EMPTY }
  );
}(this, document));

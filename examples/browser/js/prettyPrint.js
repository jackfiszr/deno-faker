...
var prettyPrint = (function() {
  ...
  var util = {
    ...
    row: function(cells, type, cellType) {
      ...
      var colSpan = util.count(cells, null) + 1, tr = util.el("tr"), td,
        attrs = {
    ...
    table: function(headings, type) {
      ...
      return {
        ...
        addRow: function(cells, _type, cellType) {
          this.appendChild(
            util.row.call(util, cells, (_type || type), cellType)
          );
          return this;
        }
      };
    ...
    type: function(v) {
      try {
        ...
        var oType = Object.prototype.toString.call(v).match(/\s(.+?)\]/)[1]
          .toLowerCase();
        if (v.nodeType) {
          if (v.nodeType === 1) {
            return "domelement";
        ...
        if (
          /^(string|number|array|regexp|function|date|boolean)$/.test(
            oType
          )
        ) {
          return oType;
          ...
          function() {
            try {
              this.parentNode.appendChild(
                prettyPrintThis(obj, { maxDepth: 1 })
              );
            } catch (e) {
              this.parentNode.appendChild(
                util.table(["ERROR OCCURED DURING OBJECT RETRIEVAL"], "error")
    ...
    getStyles: function(el, type) {
      ...
      return util.merge(
        {},
        prettyPrintThis.settings.styles["default"][el],
        type[el]
      );
    },
    ...
    stringify: function(obj) {
      ...
      var type = util.type(obj), str, first = true;
      if (type === "array") {
        str = "[";
  ...
  var prettyPrintThis = function(obj, options) {
    ...
    var settings = util.merge({}, prettyPrintThis.config, options),
      container = util.el("div"), config = prettyPrintThis.config,
      currentDepth = 0, stack = {}, hasRunOnce = false;
    ...
    var typeDealer = {
      ...
      domnode: function(node) {
        ...
        var miniTable = util.table(["DOMNode", null], "domelement"),
          data = util.htmlentities(
            (node.data || "UNDEFINED").replace(/\n/g, "\\n")
          );
        miniTable
      ...
      object: function(obj, depth, key) {
        ...
        var table = util.table(["Object", null], "object"), isEmpty = true;

        for (var i in obj) {
          if (!obj.hasOwnProperty || obj.hasOwnProperty(i)) {
            var item = obj[i], type = util.type(item);
            isEmpty = false;
            try {
        ...
        var ret = (settings.expanded || hasRunOnce)
          ? table.node
          : util.expander(
            util.stringify(obj),
            "Click to show more",
    ...
    container.appendChild(
      typeDealer[(settings.forceObject) ? "object" : util.type(obj)](
        obj,
        currentDepth
      )
    );
